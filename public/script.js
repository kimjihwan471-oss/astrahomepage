// ===================== 서버 공유 데이터 =====================
const API_BASE = '';
const ADMIN_TOKEN_KEY = 'astra_admin_token';
...
async function loadDataFromServer() {
    try {
        ...
        artists = Array.isArray(data.artists) ? data.artists : DEFAULT_ARTISTS;
        schedule = Array.isArray(data.schedule) ? data.schedule : DEFAULT_SCHEDULE;
        news = Array.isArray(data.news) ? data.news : DEFAULT_NEWS;
    } catch (error) {
        console.error(error);
...
// ===================== 별빛 배경 =====================
    ...
    function quickUploadArtistImage(id, input) {
        ...
        reader.onload = (e) => {
            ...
            saveData('artists', artists).then(renderArtists).catch(error => {
                console.error(error);
                alert(error.message);
                refreshSharedData();
            });
    ...
    async function deleteItem(type, id) {
        ...
        try {
            if (type === 'artists') artists = artists.filter(i => i.id !== id);
            if (type === 'schedule') schedule = schedule.filter(i => i.id !== id);
            if (type === 'news') news = news.filter(i => i.id !== id);
            await saveData(type, type === 'artists' ? artists : type === 'schedule' ? schedule : news);
            renderAll();
        } catch (error) {
            alert(error.message);
    ...
    async function submitArtist(id) {
    ...
    async function submitSchedule(id) {
        ...
        await saveData('schedule', schedule);
        renderSchedule();
        closeFormModal();
    }
    ...
    function setAdminState(on) {
        ...
        if (on) sessionStorage.setItem('astra_admin', 'true');
            ...
            sessionStorage.removeItem('astra_admin');
            sessionStorage.removeItem(ADMIN_TOKEN_KEY);
        }
    }

    if (sessionStorage.getItem('astra_admin') === 'true' && sessionStorage.getItem(ADMIN_TOKEN_KEY)) {
        setAdminState(true);
    }
    ...
    async function submitAdminPw() {
        try {
            const result = await apiRequest('/api/login', {
                method: 'POST',
                body: JSON.stringify({ password: adminPwInput.value.trim() })
            });
    ...
    buildStarfield();
    buildConstellation();
    loadDataFromServer().then(renderAll);
